        ...
        <div className="min-h-screen bg-[#e2e4e3] text-[#1a1a1a] font-sans antialiased relative overflow-x-hidden flex flex-col items-center select-none">
            ...
            <div className="w-full max-w-[1300px] px-6 md:px-8 relative z-10 flex flex-col">
                ...
                <header className="w-full py-6 flex justify-between items-center mb-12">
                    ...
                    {/* Groupe d'actions Header (Login + Inscription) */}
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => navigate('/login')}
                            className="text-xs font-black text-gray-700 hover:text-black transition-colors px-3 py-2"
                        >
                            Connexion
                        </button>
                        <button
                            onClick={() => navigate('/onboarding/step1')}
                            className="bg-[#1a1a1a] text-white text-xs font-bold px-5 py-2.5 rounded-full hover:bg-black transition-all shadow-sm active:scale-95"
                        >
                            {hero.ctaPrimary}
                        </button>
                    </div>
                ...
                <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-24 pt-4">
                    <div className="lg:col-span-7 flex flex-col items-start">
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <button
                                onClick={() => navigate('/onboarding/step1')}
                                className="bg-[#1e5138] text-white text-xs font-bold px-6 py-3.5 rounded-full hover:bg-[#153a28] transition-all shadow-md active:scale-95 flex items-center gap-2"
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full text-left">
                        {features.steps.map((step) => (
                            <div key={step.id} className="bg-white/60 backdrop-blur-md border border-white/40 p-6 rounded-[32px] shadow-sm flex flex-col justify-between group hover:bg-white transition-all duration-300">
                                ...
                                {/* Lien "En savoir plus" -> Emmène aussi au Onboarding */}
                                <span
                                    onClick={() => navigate('/onboarding/step1')}
                                    className="text-[11px] font-bold text-[#1e5138] mt-6 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                                >
                                    Commencer maintenant <ArrowRight size={12} />
                                </span>
                            </div>
                        ))}
                    </div>
