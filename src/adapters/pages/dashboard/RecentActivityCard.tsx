...
export default function RecentActivityCard({ activity }: Props) {
    return (
        <div className="bg-white p-3.5 rounded-xl border border-gray-100 h-[250px] flex flex-col">
            ...
            {activity.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-gray-300 gap-1.5">
                    <Inbox size={20} className="opacity-60" />
                    <span className="text-[10px] font-medium text-gray-400">Rien à signaler pour le moment</span>
                </div>
            ) : (
